...
// Pull every <img> out of a post's editor content -> [{ url, alt }].
...
// "English : 5" / "Urdu | 4" -> { name, rating(0-5) }. One language per line.
const parseLanguages = (text) =>
  (text || "")
    ...
    .map((l) => {
      ...
      const rating =
        parts.length > 1
          ? Math.max(0, Math.min(5, parseInt(parts[1], 10) || 0))
          : 0;
      return name ? { name, rating } : null;
    })
    .filter(Boolean);
...
function normalize(data) {
  ...
  const experience = (data?.experiences?.nodes || []).map((n) => ({
    ...
    consultant: n.experienceFields?.consultant || "",
    client: n.experienceFields?.client || "",
    contractor: n.experienceFields?.contractor || "",
    ...
    images: extractImages(n.content),
  }));
  ...
  return {
    hero: {
      ...
      availability: pick(hero.availability, fb.hero.availability),
...
async function getLanguages() {
  try {
    const data = await gql(LANGUAGES_QUERY);
    const parsed = parseLanguages(data?.page?.about?.languages);
    return parsed; // empty array (field exists but blank) hides the block
  } catch {
    // Field not created yet -> show fallback sample so it isn't blank.
    return fallbackData.about.languages || [];
  }
}
...
export async function getPortfolio() {
  try {
    ...
    result.about.languages = await getLanguages();
    return result;
  } catch (err) {
